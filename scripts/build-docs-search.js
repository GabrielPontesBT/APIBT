...
function buildSearchableText(doc) {
  ...
  const parts = [
    ...
    normalizeWhitespace(doc.examples?.invocation?.xml),
    normalizeWhitespace(doc.examples?.invocation?.json),
    normalizeWhitespace(doc.examples?.response?.xml),
    normalizeWhitespace(doc.examples?.response?.json),
